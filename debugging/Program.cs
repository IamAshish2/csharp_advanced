...
Console.WriteLine(6 + 10 + 17 + 20 + 31 + 36 + 40 + 41);
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    int totalAmountInTill = cashTill[3] * 20 + cashTill[2] * 10 + cashTill[1] * 5 + cashTill[0];

    Console.WriteLine($"change needed {changeNeeded}. and total amount {totalAmountInTill}");

    if (changeNeeded < 0)
        throw new InvalidOperationException("InvalidOperationException: Not enough money provided to complete the transaction.");

    if (totalAmountInTill < changeNeeded)
    {
